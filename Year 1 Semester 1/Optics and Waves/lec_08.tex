...
Hence:
\[
    y = A \cos(k_1 x - \omega_1 x) + A \cos(k_2 x - \omega_2 t)
\]
...
Or briefly:
\[
    y = 2A \color{blue}\underbrace{\cos(\Delta k x - \Delta \omega t)}_\text{group} \color{red}\underbrace{\cos(k_{avg} x - \omega_{avg}t)}_\text{carrier} \color{black}
\]
...
Assuming wave speed and frequency are independent, we have:
\[
    \boxed{V = \omega_1 / k_1 \qquad V = \omega_2 / k_2}
\]
\[
    v_\text{phase} = \frac{\omega_\text{avg}}{k_\text{avg}} = \frac{(\omega_1 + \omega_2) / 2}{(k_1 + k_2) / 2} = V
\]

And for group:
\[
    v_g = \frac{(\omega_1 - \omega_2)/2}{(k_1 - k_2) / 2} = V
\]
Hence they move at the same speed. This is only true on the assumption that wave speed does not depend on frequency, which is not generally true. Depending on the circumstances, phase velocity can be greater or smaller than group velocity.
...
\[
    I = |A|^2 = (A_1 + A_2 + A_2, \ldots, A_n)^2
\]

\subsection{Adding Incoherent Waves}
We directly add intensities instead:
\[
    I = I_1 + I_2 + \ldots = A_1^2 + A_2^2 + \ldots
\]
