...
\textbf{Early Clues}
\begin{itemize}
    \item Periodic Table (Mendelev, 1869), periodic patterns in elements properties.
...
From inside the atomic radius, we treat it as a smaller point charge $Q'(r)$, where we only consider the charge inside the portion of the pudding where $a<r$, where $a$ is the current position inside the sphere and $r$ is the atomic radius.
