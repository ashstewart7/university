% !TeX root = main.tex
\lecture{8}{Thu 04 Dec 2025 09:00}{Multivariate Distributions}
...
For example, we have a particle moving with random velocity, and we want to the random variable that models kinetic energy. Kinetic energy is the new post-transformation kinetic variable - the ``induced distribution''.
...
This is a bit tricker, because it is not a bijection. Two different values of $x$ ($x = 1, 3$) both map to $z=1$. We therefore need to consider both cases:
\[P_z(z) = \begin{cases*}
    2/6 \quad z = 1 \\
    1/6 \quad z = {0, 2, 3, 4}
\end{cases*}\]
