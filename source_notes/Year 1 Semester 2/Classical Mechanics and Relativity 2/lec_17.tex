...
\section{Example II}
In an LHC experiment, a $B_0$ meson ($b \bar{d}$) with mass $5 \si{GeV / c^2}$ is produced in a lab frame momentum of $50 \si{GeV / c^2}$. The meson has proper decay time $t_0 = 2 \times 10^{-12}$s. How far does the experiment observe the meson fly before decay?

Let the range of the $B_0$ in the lab frame be:
\[
    d_\text{lab} = v_\text{lab} t_\text{lab}
\]

There is however a time dilation between the lab frame and the rest frame, hence:
\[
    t_\text{lab} = \gamma t_0
\]
\[
    \implies d_\text{lab} = \gamma v_\text{lab} t_0
\]

To find $\gamma v_\text{lab}$ note that $p_\text{lab} = \gamma m v_\text{lab} \implies \gamma v_\text{lab} = \frac{p_\text{lab}}{m}$. Hence:
\[
    d_\text{lab} = \frac{p_\text{lab} t_0}{m}
\]
...
The derivation isn't examinable, but the result for a receding source (redshift) is:
\[
    f = f_0 \sqrt{\frac{c - v}{c + v}}
\]

And for an approaching source:
\[
    f = f_0 \sqrt{\frac{c + v}{c -v}}
\]
...
\begin{enumerate}
    \item The energy-momentum invariant is extremely useful. A lot of problems are solved by equating $E^2 - p^2c^2$ before and after a reaction or a collision.
    \item If a problem requires finding $\gamma$ or $v \gamma$ it's often useful to use $p = \gamma mv$ or $E = \gamma m v^2$ so solve for them instead of manually faffing with $u$ and $c$.
    \item Given a rest frame lifetime $t_0$, the range is given by $v \gamma t_0$ or $p t_0 / m$.
\end{enumerate}
