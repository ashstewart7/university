% !TeX root = main.tex
\lecture{10}{Thu 19 Feb 2026 15:00}{Special Relativity II: Length Contraction}
...
\[
    t \approx \frac{d}{v} \approx \frac{10^{20}}{3 \times 10^8} = 3 \times 10^{11} \si{s} \approx 10^4 \si{ years}
\]
...
\begin{enumerate}
    ...
    \item They must be linear as otherwise a constant speed in one would not be constant in the other (and therefore not inertial frames).
    \item If relative velocity is only in the $x$ direction, the transformations must be independent of $y$ and $z$.
\end{enumerate}
...
\]
\[
    t = \gamma \left(t^\prime + \frac{x^\prime v}{c^2}\right)
\]
...
\]
\[
    t^\prime = \gamma \left(t - \frac{x v}{c^2}\right)
\]
